...
**注意**：根据实际需要，一个盒子下可添加多条连接，根据规约类型及盒子类型选择连接方式。
...
注：需首先选择同一种设备，此后才能为这些设备选择设备模板，如下图所示：
...
点击设备后的![](media/image016.png)按钮可以从连接中删除此设备。若需批量删除设备，则勾选此连接下需要删除的设备，而后点击批量删除按钮即可，如下图所示：
...
1. 逐个设备进行配置
   ...
   点击设备后的![](media/image018.png)图标，可对此台设备进行编辑，如下图所示：
...
**注意**：导出的表中支持AI，DI，PI，AO，DO，PO偏移量的配置。基本的配置方式为用短划线连接，如0-50，当同时存在多个偏移量时，可用\#隔离，如0-50\#1000-1050。
